...
const SelectMember = () => {
  ...
  useEffect(() => {
    if (isMount.current) {
      ...
      if (
        userObj.presidentOf !== null &&
        userObj.presidentOf !== "" &&
        userObj.presidentOf !== undefined
      ) {
        setClubs((prevStat) => {
          return {
            ...prevStat,
      ...
      if (
        userObj.rolesOf !== null &&
        userObj.rolesOf !== "" &&
        userObj.rolesOf !== undefined
      ) {
        setClubs((prevStat) => {
          return {
            ...prevStat,
            [userObj.rolesOf.name]: true,
          };
        });
      }
    } else {
        ...
        <div className="px-5">
          <div className=" flex flex-row justify-start items-center mt-2">
            <div className="bg-white rounded-full w-44 h-44 relative overflow-hidden">
              <Image
                src={
                  userObj.imageObj != undefined
                    ? userObj.imageObj.secure_url
                    : "/portrait-1.jpg"
                }
                layout="fill"
                objectFit="cover"
          ...
          <div>
            <span>Add user as {selectedClubName} Precedent </span>
            <ToggleButton
              value={precedent}
              setValue={setPrecedent}
              forId="club Precedent"
            />
          </div>
