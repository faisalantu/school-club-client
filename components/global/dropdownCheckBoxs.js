...
export default function dropdownCheckBoxs({ label = '', type, dropdownItems = {}, editMode = false, changeState }) {
    ...
    return (<div className='flex flex-col mb-1' >
        ...
        <div className=' relative '>
            <div className='   cursor-pointer flex flex-col ' onClick={() => { editMode && setShow(!show) }}>
                <select className={` rounded-lg border ${editMode ? 'bg-gray-300' : 'bg-gray-100 text-gray-300 '}  py-2 px-3 text-grey-darkest outline-none`} >
                    <option>{selectData.map((str,index) => {return(`${ (index !==0) ? ',':'' } ${str} `)})}</option>
